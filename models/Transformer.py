...
from .transformer_block import MutualSelfBlock, LocalAttentionBlock
...
class TransformerEncoder(nn.Module):
    def __init__(self, depth, num_heads, embed_dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
        ...
        self.blocks2 = nn.ModuleList([
                 ...
                 drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer)
                                        for i in range(depth)])
        self.rgb_red = nn.Linear(384*2,384)
        self.depth_red = nn.Linear(384*2,384)
        self.rgb_norm2 = norm_layer(embed_dim)
        self.depth_norm2 = norm_layer(embed_dim)
        #self.chanel_att = SELayer(channel=384*2)
    ...
    def forward(self, rgb_fea, depth_fea):
        ...
        for block in self.blocks2:
            rgb_fea,depth_fea = block(rgb_fea,depth_fea)
        rgb_fea = self.rgb_norm2(rgb_fea)
        depth_fea = self.depth_norm2(depth_fea)
...
class token_Transformer(nn.Module):
    def __init__(self, embed_dim=384, depth=14, num_heads=6, mlp_ratio=3.):
        ...
        self.mutualinfo = Mutual_info_reg(input_channels=384, channels=64, latent_size=6)
    def forward(self, rgb_fea, depth_fea):
        
        B, _, _ = rgb_fea.shape
        loss = self.mutualinfo(rgb_fea,depth_fea)
        fea_1_16 = torch.cat([rgb_fea, depth_fea], dim=2)
        #fea_1_16 = self.CA(fea_1_16)
        ...
        return saliency_fea_1_16, fea_1_16, saliency_tokens,loss
